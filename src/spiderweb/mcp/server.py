...
async def list_tools():
    return [
        # ── L1: document layer ──
        Tool(name="doc_ingest", description="Ingest a document (.md/.txt/.epub). Async: L1 fast return with task_id, vector+graph_build run in background. Poll task_status or watch notifications for progress.", inputSchema={"type": "object", "properties": {"file_path": {"type": "string", "description": "Absolute path to the document file"}, "title": {"type": "string", "description": "Optional title override"}, "author": {"type": "string", "description": "Optional author"}}}),
